...
const RUNTIME_NAME = 'mediapipe';
const TEST_PAGE_URL = 'http://localhost:3004';
const MODEL_URL = 'https://huggingface.co/litert-community/Qwen2.5-0.5B-Instruct/resolve/main/Qwen2.5-0.5B-Instruct_seq128_q8_ekv1280.tflite';

test.describe('@mediapipe/tasks-genai Tests', () => {
  ...
  test('Basic performance test', async ({ page }) => {
    ...
    const loadSuccess = await page.evaluate(async (modelUrl) => {
      return await window.testAPI.loadModel(modelUrl);
    }, MODEL_URL);
